...
void loop() 
{
 ...
 if(digitalRead(BTN)==0)
 {
  ...
  for(int i=0;i<10;i++)
  {
     ...
     {
        sprite1.drawString( cc[i]
                          , x[start[i]+angle]
                          , y[start[i]+angle] );
        
        sprite1.drawLine( px[start[i]+angle]
                        , py[start[i]+angle]
                        , lx[start[i]+angle]
                        , ly[start[i]+angle]
                        , color1 );
     ...
     else
     {
        ...
        sprite1.drawLine( px[(start[i]+angle)-360]
                        , py[(start[i]+angle)-360]
                        , lx[(start[i]+angle)-360]
                        , ly[(start[i]+angle)-360]
                        , color1 );
     }
  }
  ...
  for(int i=0; i<60; i++)
  {
    if(startP[i] + angle<360)
        sprite1.fillCircle( px[startP[i]+angle]
                          , py[startP[i]+angle]
                          , 1
                          , color1);  //--- TFT_WHITE
    else
        sprite1.fillCircle( px[(startP[i]+angle)-360]
                          , py[(startP[i]+angle)-360]
                          , 1
                          , color1);  //--- TFT_WHITE
  }
  //--- update gauge inner text area
  sprite1.setTextColor(TFT_SILVER, color6);   //--- light_grey
  sprite1.drawString( "AMBIENT"
                    , sx
                    , sy-22
                    , 2 );

  sprite1.drawString( "LIGHT"  
                    , sx
                    , sy+18
                    , 2 );  //--- font

  sprite1.setTextColor(color1, color6);  //---fg,bg

  sprite1.drawString( String( (int)v) + "%"
                    , sx
                    , sy
                    , 4 ); //---font

  sprite1.fillTriangle( 70   //x1
                      , 28   //y1
                      , 74   //x2
                      , 36   //y2
                      , 66   //x3
  ...
  for(int i=1; i<6; i++)
  {
      ...
      if(i==1 || i==3 || i==5)
      {   
          sprite2.drawString( String(i*9-27)
                            , gx-12
                            , gy-(i*9) );
      }
  }
  ...
  if(curentTime+period<millis())
  {
    ...
    for(int i=0;i<6;i++)
    {
        sprite3.drawString( String(t[i]) + "  BATT=" + String(term[i]) + "     "
                            , 4     //x
                            , 4 + i * 11 ); //y
        c1=!c1;
    }
  ...
  sprite4.drawCircle(120,28,10,TFT_SILVER);   //x,y,r,color
